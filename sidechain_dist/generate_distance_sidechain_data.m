...
%   Detailed explanation goes here
    ...
    load_output = load('sidechain_dist/amino_acid_names.mat', 'names');
    ...
    if(~isempty(sidechain_atom_distances{amino_index, domain_index}))
        fprintf("Atoms Mean Position: (%.2f,%.2f,%.2f)\n", all_coords_mean(1), all_coords_mean(2), all_coords_mean(3));
        fprintf("Distance Mean: %.2f (%.2f%%)(%f,%f)\n", all_distances_mean, (data_output.count.mean/data_output.count.total)*100.00, data_output.count.mean, data_output.count.total);
        fprintf("Distance Quartiles: %.2f,%.2f,%.2f\n", quartiles(1),quartiles(2),quartiles(3)); 
        fprintf("Quartiles Count: %.2f,%.2f,%.2f\n", data_output.count.first_quartile, data_output.count.median, data_output.count.third_quartile); 
        fprintf("Distance STD: %.2f\n", std(all_distances));
        fprintf("Residue Mean Distance: %.2f\n",data_output.distances.residue_level_mean); 
    end
